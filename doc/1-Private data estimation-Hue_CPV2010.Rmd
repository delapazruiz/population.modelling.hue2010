...
Section 2.1 introduce the target of the algorithm which is the base of an evaluation. The evaluation applies for the `age-ranges and gender`, `school`, and `work` related variables. The results evaluation demonstrate that the algorithm target is fulfilled considering the following criteria:
...
------------- | -------------
Known value of a targeted variable  | Defined with the sum of estimated values of the many blocks that compound the target variables (sum by columns)
...
```{r}
(hpcons.estima$P_0A2.f+
 hpcons.estima$P_3A5.f+
 hpcons.estima$P_6A11.f+
 hpcons.estima$P_12YMAS.f) %>% as.data.frame()
...
join(
   ...
   as.data.frame()%>% colnames()%>% c("estimation")
   
   
   %>% 
   ),
  (hpcons.orig$POBTOT%>% as.data.frame(col.names = "observation"))
  )

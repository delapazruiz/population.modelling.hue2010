## ---------------------------#
##
## Script name: 04-Huehuetla_SMS_into_Netlogo
##
## Purpose of script: After running the SMS, this scripts prepares agents to
##                    import them into Netlogo.
##
...
##
## Pre-requirement R scripts to generate input data:
...
#CONSTRAINS Importing raw census data, updating column names, and fill NA vaulues ---------------------------------
...
source("code/Huehuetla_CPV2010_Data privacy algorithm.R") # before named h_hpcons_2010.R
sum(hpcons1)
sum(hpcons2)
sum(hpcons3)
...
# Loading Individual survey data (Encuesta Intercensal 2015): Variable "hind" ---------------------------------
...
source("code/Huehuetla_Intercensal2015_data preparation.R")
...
# CREATE WEIGHT MATRIX - All zones together ---------------------------------
...
# INTEGERISATION AND EXPANSION WITH MIPFP ---------------------------------
...
# EVALUATION: Simulation vs observation---------------------------------
...
# Plotting results: simulated vs observations(census-data)---------------------------------

#Plot of global correlations of the constrains
plot(as.numeric(as.matrix(hpcons3)), as.numeric(shpcons3),
     main="Count of individuals in a block based on work assistance",
     ylab="Simulated-Individuals",
     xlab="Observed-Census",
     col = c("red", "blue"),
     pch = c(1, 3),
    ...
    legend("topleft",
           c("Census","individual Simulation"),
           fill=c("red","blue")
    )

plot(as.numeric(as.matrix(hpcons2)), as.numeric(shpcons2),
     main="Count of individuals in a block based on school assistance",
     ylab="Simulated-Individuals",
     xlab="Observed-Census",
     col = c("red", "blue"),
     pch = c(1, 3),
...
plot(as.numeric(as.matrix(hpcons1_conv)), as.numeric(shpcons1),
     main="Count of individuals in a block based on Age & Sex",
     ylab="Simulated-Individuals",
     xlab="Observed-Census",
     col = c("red", "blue"),
     pch = c(1, 3),
...
# Correlation GLOBAL & VARIABLES: simulated vs observations---------------------------------
...
# Correlation by BLOCK: simulated vs observations(census-data)---------------------------------
...
# Maximum errors: simulated vs observations(census-data) ---------------------------------
...
# SMS into Netlogo: Creating "sms.agent.csv" ---------------------------------

library(stringr)

colnames(hpconsfull[,(1:9)])

hpconsfull$CVEGEO <- paste(hpconsfull$Clave.de.entidad.federativa,
      str_pad(hpconsfull$Clave.de.municipio.o.delegaci?n, 3, pad = "0"),
      str_pad(hpconsfull$Clave.de.localidad,4, pad = "0"),
      str_pad(hpconsfull$Clave.de.ageb,4, pad = "0"),
      str_pad(hpconsfull$Clave.de.manzana,3, pad = "0"),
      sep = "")

hpconsfull$Block <- seq.int(nrow(hpconsfull))

indiv_mipfp.id <- merge(indiv_mipfp, hpconsfull[,c("Block","CVEGEO")], by = "Block",
                        all = TRUE)

indiv_mipfp.id$ind.id <- str_pad(seq.int(nrow(indiv_mipfp.id)),5, pad = 0)



#2 EXPORT AGENTS FOR NETLOGO

indiv_mipfp.id <- indiv_mipfp.id [,c(
  "Sex","Age","Go_School","Go_Work","Escolar_grade","Escolar_level","CVEGEO","ind.id")]

# write.table(indiv_mipfp.id,
#             "NetLogo/sms_wastewater/data/sms_agent.csv", 
#             row.names=FALSE, col.names=FALSE, sep=",")


# Select and match Block.shp with sms_agent.csv ---------------------------------
...
manzanas_hue <- readOGR("NetLogo/sms_wastewater/data/manzana_loc_huehuetla.shp")
households_id <- readOGR("NetLogo/sms_wastewater/data/idhouseholds.shp")

#Check the data
head(manzanas_hue@data)
colnames(manzanas_hue@data)

head(households_id@data)

#Plot maps
plot(manzanas_hue)
plot(households_id)

# Verify ID matches manzanas V.S. individuals
setdiff(indiv_mipfp.id$CVEGEO, manzanas_hue$CVEGEO)
is.element(manzanas_hue$CVEGEO, indiv_mipfp.id$CVEGEO)

# Verify ID matches households V.S. individuals
setdiff(indiv_mipfp.id$CVEGEO,  households_id$CVEGEO)
is.element(households_id$CVEGEO, indiv_mipfp.id$CVEGEO)
unique(households_id$CVEGEO)

#How many individual uniq ID's are inside of Boclk ID's
sum(is.element(manzanas_hue$CVEGEO, indiv_mipfp.id$CVEGEO))

#How many unique ID's individual exist
unique(indiv_mipfp.id$CVEGEO)
unique(indiv_mipfp.id$CVEGEO)

#Match with blocks ID that exist
indiv_mipfp.id$CVEGEO <- replace(
  indiv_mipfp.id$CVEGEO, 
  indiv_mipfp.id$CVEGEO == "130270001011A007","130270001011A008")

indiv_mipfp.id$CVEGEO <- replace(
  indiv_mipfp.id$CVEGEO, 
  indiv_mipfp.id$CVEGEO == "1302700010105030","1302700010105031")

indiv_mipfp.id$CVEGEO <- replace(
  indiv_mipfp.id$CVEGEO, 
  indiv_mipfp.id$CVEGEO == "1302700010105031","1302700010105029")

write.table(indiv_mipfp.id,
            "NetLogo/sms_wastewater/data/sms_agent.csv", 
            row.names=FALSE, col.names=FALSE, sep=",")

# Economic activities (school & workers): Match manzana_loc_huehuetla.shp with sms_agent.csv ---------------------------------

#Mobility: Read School and working places
economicpoints_hue <- readOGR("NetLogo/sms_wastewater/data/economicpoints_huehuetla.shp")

#Add col economic CVEGEO
economicpoints_hue$CVEGEO <- paste(economicpoints_hue$cve_ent, economicpoints_hue$cve_mun, economicpoints_hue$cve_loc, economicpoints_hue$ageb, economicpoints_hue$manzana, sep = "")

#Explore the names of the school categories and range of workers 
sort(unique(economicpoints_hue$nombre_act))
sort(unique(economicpoints_hue$per_ocu))

#Avg worker number by economic activity
avg.workers.list <- c(3, 7, 21, 40, 76, 270)
school.exist.list <- c("school.pre","school.elementary","school.high","school.multilevel")

#Avg Number of workers and school locations (colum)
economicpoints_hue$avg.workers <- 
  avg.workers.list[match(economicpoints_hue$per_ocu, 
                          c("0 a 5 personas",
                            "6 a 10 personas",
                            "11 a 30 personas",
                            "31 a 50 personas",
...
economicpoints_hue$school.exist <-
  school.exist.list[match(economicpoints_hue$nombre_act,
                          c( 
...
#Convert to SF data to work with spatial Tidyverse
economicpoints_hue.sf = st_as_sf(economicpoints_hue)

#Aggregate workers by block ID
economicpoints_hue.work = economicpoints_hue.sf %>%
  group_by(CVEGEO) %>%
  summarise(avg.workers.cvegeo = sum(avg.workers))%>% 
  dplyr::select(1,2)

#Aggregate schools levels by block ID
economicpoints_hue.school = economicpoints_hue.sf %>%
  group_by(CVEGEO)%>% dplyr::select(42, 44) %>% dcast(CVEGEO ~ school.exist)%>% 
  dplyr::select(1,2,3,4,5)

#Aggregated Merge workers and schools in blocks shape and export dbf ---------------------------------

manzanas_hue.workschool <- merge(manzanas_hue, economicpoints_hue.school, by="CVEGEO")
manzanas_hue.workschool <- merge(manzanas_hue.workschool, economicpoints_hue.work, by="CVEGEO")

manzanas_hue.workschool@data[["school.pre"]][is.na(manzanas_hue.workschool@data[["school.pre"]])] <- 0
manzanas_hue.workschool@data[["school.elementary"]][is.na(manzanas_hue.workschool@data[["school.elementary"]])] <- 0
manzanas_hue.workschool@data[["school.high"]][is.na(manzanas_hue.workschool@data[["school.high"]])] <- 0
manzanas_hue.workschool@data[["school.multilevel"]][is.na(manzanas_hue.workschool@data[["school.multilevel"]])] <- 0
manzanas_hue.workschool@data[["avg.workers.cvegeo"]][is.na(manzanas_hue.workschool@data[["avg.workers.cvegeo"]])] <- 0


#manzanas_hue.workschool <- as.data.frame(manzanas_hue.workschool)
manzanas_hue.workschool <- as.data.frame(manzanas_hue.workschool)
manzanas_hue.workschool = manzanas_hue.workschool %>% dplyr::select(1,32,33,34,35,36)

manzanas_hue.workschool$avg.workers.cvegeo <- as.integer(manzanas_hue.workschool$avg.workers.cvegeo)
manzanas_hue.workschool$school.pre <- as.integer(manzanas_hue.workschool$school.pre)
manzanas_hue.workschool$school.elementary <- as.integer(manzanas_hue.workschool$school.elementary)
manzanas_hue.workschool$school.high <- as.integer(manzanas_hue.workschool$school.high)
manzanas_hue.workschool$school.multilevel <- as.integer(manzanas_hue.workschool$school.multilevel)

write.csv(manzanas_hue.workschool, "manzanas_hue.workschool.csv")
write.dbf(manzanas_hue.workschool, "manzanas_hue.workschool.dbf")

# No aggregated Economic activities (school & workers):  ---------------------------------

#Clean NA, and interger vaules from economic activities (school) 
economicpoints_hue@data[["school.exist"]][is.na(economicpoints_hue@data[["school.exist"]])] <- "no"
economicpoints_hue$avg.workers <- as.integer(economicpoints_hue$avg.workers)

write.dbf(economicpoints_hue, "economicpoints_hue.dbf")
